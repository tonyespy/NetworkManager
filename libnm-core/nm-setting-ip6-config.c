...
verify (NMSetting *setting, NMConnection *connection, GError **error)
{
	...
	addr_gen_mode = nm_setting_ip6_config_get_addr_gen_mode (NM_SETTING_IP6_CONFIG (setting));
	if (   !addr_gen_mode
	    || !strcmp (addr_gen_mode, NM_SETTING_IP6_CONFIG_ADDR_GEN_MODE_EUI64)
	    || !strcmp (addr_gen_mode, NM_SETTING_IP6_CONFIG_ADDR_GEN_MODE_STABLE_PRIVACY))
		return TRUE;
              ...
              const GValue *value, GParamSpec *pspec)
{
	...
	case PROP_ADDR_GEN_MODE:
		g_free (priv->addr_gen_mode);
		priv->addr_gen_mode = g_value_dup_string (value);
		break;
	default:
              ...
              GValue *value, GParamSpec *pspec)
{
	...
	case PROP_IP6_PRIVACY:
		...
		break;
	case PROP_ADDR_GEN_MODE:
		g_value_set_string (value, priv->addr_gen_mode);
		break;
	default:
		G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
		break;
...
nm_setting_ip6_config_class_init (NMSettingIP6ConfigClass *ip6_class)
{
	...
	g_object_class_install_property
		(object_class, PROP_ADDR_GEN_MODE,
		 g_param_spec_string (NM_SETTING_IP6_CONFIG_ADDR_GEN_MODE, "", "",
		                      NULL,
		                      G_PARAM_READWRITE |
