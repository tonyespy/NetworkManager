                                 ...
                                 GError **error)
{
	...
	if (!in_password) {
		if (!crypto_randomize (pw_buf, sizeof (pw_buf), error))
			return NULL;
		in_password = tmp_password = nm_utils_bin2hexstr ((const guint8 *) pw_buf, sizeof (pw_buf), -1);
	}
	...
	tmp = nm_utils_bin2hexstr ((const guint8 *) salt, salt_len, salt_len * 2);
 ...
 */
char *
nm_utils_bin2hexstr (const guint8 *bytes, gsize len, int final_len)
{
