...
machine_secret ()
{
	...
	if (!g_file_get_contents ("/etc/machine-secret", &machine_secret_hex, NULL, NULL))
		nm_log_info (LOGD_CORE, "could not read a machine secret");
	...
	if (read (urandom, machine_secret, MACHINE_SECRET_SIZE) != MACHINE_SECRET_SIZE) {
		nm_log_err (LOGD_CORE, "can't generate machine secret from /dev/urandom");
		g_clear_pointer (&machine_secret, g_free);
		goto out;
	}
	...
	if (!g_file_set_contents ("/etc/machine-secret", machine_secret_hex, -1, NULL)) {
		nm_log_err (LOGD_CORE, "could not save a machine secret");
		g_clear_pointer (&machine_secret, g_free);
		goto out;
	}
