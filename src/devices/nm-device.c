...
dhcp4_cleanup (NMDevice *self, CleanupType cleanup_type, gboolean release)
{
	...
	if (priv->dhcp4.client) {
		...
		if (priv->dhcp4.state_sigid) {
			g_signal_handler_disconnect (priv->dhcp4.client, priv->dhcp4.state_sigid);
			priv->dhcp4.state_sigid = 0;
		}
...
dhcp4_fail (NMDevice *self, gboolean timeout)
{
	...
	_LOGD (LOGD_DEVICE, "DHCPv4 failed: timeout %d, num tries left %u",
...
dhcp6_cleanup (NMDevice *self, CleanupType cleanup_type, gboolean release)
{
	...
	if (priv->dhcp6.client) {
		if (priv->dhcp6.state_sigid) {
			g_signal_handler_disconnect (priv->dhcp6.client, priv->dhcp6.state_sigid);
			priv->dhcp6.state_sigid = 0;
		}
...
dhcp6_fail (NMDevice *self, gboolean timeout)
{
	...
	_LOGD (LOGD_DEVICE, "DHCPv6 failed: timeout %d, num tries left %u",
