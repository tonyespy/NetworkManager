...
static void
nm_device_queued_ip4_config_change_clear (NMDevice *self)
{
	NMDevicePrivate *priv = NM_DEVICE_GET_PRIVATE (self);
...
static void
nm_device_queued_ip6_config_change_clear (NMDevice *self)
{
	NMDevicePrivate *priv = NM_DEVICE_GET_PRIVATE (self);
...
nm_device_activate_schedule_ip4_config_result (NMDevice *self, NMIP4Config *config)
{
	...
	nm_device_queued_ip4_config_change_clear (self);
...
_cleanup_ip6_pre (NMDevice *self, CleanupType cleanup_type)
{
	...
	nm_device_queued_ip6_config_change_clear (self);
...
static gboolean
nm_device_reapply_ip6_config (NMDevice *self,
                              NMConnection *old,
                              gboolean reconfigure,
                              GError **error)
{
                              ...
                              GError **error)
{
	...
	while (g_hash_table_iter_next (&iter, (gpointer *)&setting, NULL)) {
		if (strcmp (setting, NM_SETTING_IP4_CONFIG_SETTING_NAME) == 0) {
			...
			if (!nm_device_reapply_ip6_config (self, old, reconfigure, error))
				goto done;
            ...
            gpointer user_data)
{
	...
	if (priv->state != NM_DEVICE_STATE_ACTIVATED) {
		...
		g_error_free (local);
	} else if (!nm_device_reapply_connection (self,
	                                          info->connection,
	                                          info->flags,
	                                          FALSE,
	                                          &local)) {
                     ...
                     guint flags)
{
	...
	s_con = nm_device_get_settings_connection (self);
	g_assert (s_con);
	...
	if (settings && g_variant_n_children (settings)) {
		...
		connection = NM_CONNECTION (g_object_ref (s_con));
	}
